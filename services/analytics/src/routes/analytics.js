...
analyticsRouter.post('/analytics/jobs/top', async (req, res, next) => {
  try {
    ...
    return res.json(ok({ metric, window_days, jobs }, req.traceId));
  } catch (err) {
    next(err);
  }
});
...
analyticsRouter.post('/analytics/funnel', async (req, res, next) => {
  try {
    ...
    return res.json(ok({ job_id, window_days, view, save, apply_start, submit, rates }, req.traceId));
  } catch (err) {
    next(err);
  }
});
...
analyticsRouter.post('/analytics/geo', async (req, res, next) => {
  try {
    ...
    return res.json(ok({ job_id, window_days, cities }, req.traceId));
  } catch (err) {
    next(err);
  }
});
...
analyticsRouter.post('/analytics/member/dashboard', async (req, res, next) => {
  try {
    ...
    // 1) profile_views grouped by ISO day
    ...
    // 2) application status breakdown from MySQL
