#!/usr/bin/env bash

set -euo pipefail

BASE_URL="${1:-http://localhost}"

echo "Using base URL: ${BASE_URL}"

echo
echo "== Health checks =="
if [[ "${BASE_URL}" == http://localhost* || "${BASE_URL}" == http://127.0.0.1* ]]; then
  curl -fsS "${BASE_URL}:8001/health" | python3 -m json.tool
  curl -fsS "${BASE_URL}:8002/health" | python3 -m json.tool
  curl -fsS "${BASE_URL}:8003/health" | python3 -m json.tool
else
  curl -fsS "${BASE_URL}/members/search" -H 'content-type: application/json' -d '{"page":1,"page_size":1}' | python3 -m json.tool >/dev/null
  echo "ALB endpoint reachable"
fi

echo
echo "== Profile search =="
if [[ "${BASE_URL}" == http://localhost* || "${BASE_URL}" == http://127.0.0.1* ]]; then
  curl -fsS "${BASE_URL}:8001/members/search" \
    -H 'content-type: application/json' \
    -d '{"page":1,"page_size":5}' | python3 -m json.tool
else
  curl -fsS "${BASE_URL}/members/search" \
    -H 'content-type: application/json' \
    -d '{"page":1,"page_size":5}' | python3 -m json.tool
fi

echo
echo "== Job search =="
if [[ "${BASE_URL}" == http://localhost* || "${BASE_URL}" == http://127.0.0.1* ]]; then
  curl -fsS "${BASE_URL}:8002/jobs/search" \
    -H 'content-type: application/json' \
    -d '{"page":1,"page_size":5}' | python3 -m json.tool
else
  curl -fsS "${BASE_URL}/jobs/search" \
    -H 'content-type: application/json' \
    -d '{"page":1,"page_size":5}' | python3 -m json.tool
fi

echo
echo "== Application list by member =="
if [[ "${BASE_URL}" == http://localhost* || "${BASE_URL}" == http://127.0.0.1* ]]; then
  curl -fsS "${BASE_URL}:8003/applications/byMember" \
    -H 'content-type: application/json' \
    -d '{"member_id":"22222222-2222-2222-2222-222222222222","page":1,"page_size":5}' | python3 -m json.tool
else
  curl -fsS "${BASE_URL}/applications/byMember" \
    -H 'content-type: application/json' \
    -d '{"member_id":"22222222-2222-2222-2222-222222222222","page":1,"page_size":5}' | python3 -m json.tool
fi

echo
echo "Backend smoke test completed."
